...
function GetMusicList(id, page) {
	...
	$.ajax({
		...
		success: function (data) {
	        ...
	        disk_img.src = object.data[0].AlbumImgUrl;
	        song_img.src = object.data[0].AlbumImgUrl;

            if (account != "") {
	                $(".gc").css("background", "url(" + object.data[0].AlbumImgUrl + ")");
...
function Sensitive(content) {
	var sensitive_txt = new Array("广告", "色情", "违法", "网址", "政治");
	for (var i = 0; i < sensitive_txt.length; i++) {
		var fileUrl = "../Content/Sensitive/" + sensitive_txt[i] + ".txt";
		var htmlobj = $.ajax({ url: fileUrl, async: false });
		var arr = htmlobj.responseText.split(/\r?\n/);
		for (var x = 0; x < arr.length; x++) {
			if (content.indexOf(arr[x]) >= 0 && arr[x] != "") {
				layer.msg('包含敏感词汇：' + arr[x] + "！", { icon: 2 });
				return false;
            }
		}
	}
